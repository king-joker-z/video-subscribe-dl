...
wave: 1
autonomous: true
objective: Re-enable authentication by default and replace WebSocket ?token= query-param exposure with short-lived session nonces (POST /api/session → ?session=<nonce>).
