...
func TestMatchesFilter(t *testing.T) {
	...
	for _, tt := range tests {
		got := filter.MatchesSimple(tt.title, tt.f)
		if got != tt.want {
			t.Errorf("MatchesSimple(%q, %q) = %v, want %v", tt.title, tt.f, got, tt.want)
		}
	}
}
...
func TestParseFilterRules_Empty(t *testing.T) {
	if rules := filter.ParseRules(""); rules != nil {
...
func TestParseFilterRules_InvalidJSON(t *testing.T) {
	if rules := filter.ParseRules("not json"); rules != nil {
...
func TestMatchesFilterRules_EmptyRules(t *testing.T) {
	if !filter.MatchesRules(nil, filter.VideoInfo{Title: "anything"}) {
		t.Error("expected true for nil rules")
	}
	if !filter.MatchesRules([]filter.Rule{}, filter.VideoInfo{Title: "anything"}) {
...
func TestMatchesFilterRules_TitleContains(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Title: "Learn Golang Today"}) {
		t.Error("expected match for title containing 'golang'")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Title: "Learn Python Today"}) {
...
func TestMatchesFilterRules_TitleNotContains(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Title: "正片内容"}) {
		t.Error("expected match for title not containing '广告'")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Title: "这是广告视频"}) {
...
func TestMatchesFilterRules_TitleRegex(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Title: "EP01 First Episode"}) {
		t.Error("expected match for regex EP01")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Title: "No match here"}) {
...
func TestMatchesFilterRules_TitleRegex_Invalid(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Title: "anything"}) {
...
func TestMatchesFilterRules_DurationGt(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Duration: 600}) {
		t.Error("expected match for duration > 300")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Duration: 200}) {
...
func TestMatchesFilterRules_DurationLt(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Duration: 30}) {
		t.Error("expected match for duration < 60")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Duration: 120}) {
...
func TestMatchesFilterRules_DurationBetween(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Duration: 60}) {
		t.Error("expected match for duration == 60 (inclusive)")
	}
	if !filter.MatchesRules(rules, filter.VideoInfo{Duration: 300}) {
		t.Error("expected match for duration == 300 (inclusive)")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Duration: 10}) {
		t.Error("expected no match for duration < 60")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Duration: 500}) {
		t.Error("expected no match for duration > 300")
	}
}
...
func TestMatchesFilterRules_PagesGt(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Pages: 5}) {
		t.Error("expected match for pages > 1")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Pages: 1}) {
...
func TestMatchesFilterRules_TagsContains(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Tags: "golang,tutorial,programming"}) {
		t.Error("expected match for tags containing 'golang'")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Tags: "python,java"}) {
...
func TestMatchesFilterRules_MultipleRules_AND(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Title: "Go教程", Duration: 600}) {
		t.Error("expected match when both rules pass")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Title: "Go教程", Duration: 100}) {
		t.Error("expected no match when duration rule fails")
	}
	if filter.MatchesRules(rules, filter.VideoInfo{Title: "Go入门", Duration: 600}) {
...
func TestMatchesFilterRules_InvalidNumericValue(t *testing.T) {
	...
	if !filter.MatchesRules(rules, filter.VideoInfo{Duration: 100}) {
