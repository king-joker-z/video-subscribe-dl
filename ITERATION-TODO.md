...
> 最后更新: 2026-03-18（fix(ui): 卡片视图批量选择 + FAB 位置修复 — ba1eb16）
...
- [x] 卡片视图批量选择 checkbox（封面左上角覆盖层）+ 全选当页按钮 + FAB 手机端位置修复 — ba1eb16
...
- [x] **视频卡片升级** <!-- commit: 7c3c007 feat(ui): P2 视频卡片升级 - 平台 logo 兜底 + 状态 badge tooltip + 下载进度条优化 -->
  ...
  - 进度条样式优化，下载中状态更明显
  - 状态 badge 加 tooltip
- [x] **侧边栏 LOGO 优化**（"V" 太简陋）<!-- commit: 2818d10 -->
  - 蓝紫渐变背景（#3b82f6 → #6366f1），内联 SVG 图标
  - 图标含：显示器外框、黑色屏幕、播放三角、进度条（半满）、订阅弧线装饰
  - 展开态标题增加 tracking-wide，视觉更精致
- [x] **Skeleton 精细化**（模拟真实卡片形状）<!-- commit: 21f2277 feat(ui): P2 Skeleton 精细化 - 各页面骨架屏匹配真实卡片形状 -->
- [x] **全局下载进度浮动条**（顶部/底部，X/Y 下载中）<!-- commit: 18cfd15 -->

### P3 — 功能完善

- [x] **视频详情页增强** <!-- commit: 2d8b002 -->
  - 显示文件大小 + 时长 + 分辨率（从 file_path 推断或存 DB）
  - 下载中状态实时进度条（接 SSE）
- [x] **订阅源"立即检查"按钮** <!-- commit: 1800458 -->
  - 目前只能等定时触发，手动 trigger 一次检查很有用
  - 卡片底部信息栏增加"立即检查"文字按钮，带 loading spinner
  - 点击后 2s 自动刷新列表，同时支持多卡并发检查
- [x] **批量下载进度汇总** <!-- commit: 18cfd15（已由 P2 GlobalDownloadBar 覆盖，SSE 驱动，展示 X 个视频下载中 + 均速 + 进度条）-->
  - 顶部全局进度条（X/Y 视频下载中）
